...
const SERVER_NAME: &str = "argus-mcp";
...
fn run_stdio(config: ServerConfig) -> Result<()> {
    ...
    {
        let _ = config;
        bail!("argus-mcp requires the Unix socket API, which is only available on Unix platforms")
    }
}
...
usage: argus-mcp [--socket <path>]

Options:
  --socket <path>  Connect to an Argus daemon Unix socket at <path>
  -h, --help       Print this help text

By default argus-mcp connects to the same Unix socket path as argus-daemon.";
...
fn tool_definitions() -> Value {
    json!([
        {
            "name": "list_sessions",
            "title": "List Argus Sessions",
            "description": "List daemon-owned Argus sessions with current snapshots.",
            "inputSchema": {
                "type": "object",
                "properties": {}
            },
        ...
        },
        {
            ...
            "title": "Read Argus Session Snapshot",
            "description": "Read the current daemon snapshot for one Argus session.",
            "inputSchema": {
                "type": "object",
                "properties": {
                    "session_id": {
        ...
        {
            "name": "styled_rows",
            "title": "Read Argus Styled Rows",
            "description": "Read a styled row range from one Argus session snapshot.",
            "inputSchema": {
                "type": "object",
                "properties": {
                    "session_id": {
...
mod tests {
    ...
    impl SessionApi for RecordingApi {
        ...
        fn start_session(
            &self,
            _request: argus_core::session::StartSessionRequest,
        ) -> Result<SessionId> {
            unimplemented!()
        }
        ...
        fn release_input_lease(
            ...
            _client_id: argus_core::session::ClientId,
        ) -> Result<LeaseChange> {
            unimplemented!()
        }

        fn write_input(&self, _request: argus_core::session::WriteInputRequest) -> Result<()> {
            unimplemented!()
        }
        ...
        ) -> Result<argus_core::session::StyledRowsResponse> {
            Ok(argus_core::session::StyledRowsResponse {
