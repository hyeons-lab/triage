...
4. **Strictly bounded timeout (10s)**: It enforces its own internal 10-second timeout (comfortably within typical agent hook timeouts of 15s to 30s), so it returns `ask` cleanly rather than being terminated mid-execution.
