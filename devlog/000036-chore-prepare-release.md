...
- **Agent:** Antigravity
- **Intent:** Configure workspace-level and crate-specific packaging metadata, descriptions, license inheritance, keywords, categories, and documentation URLs, and add individual package-level READMEs to prepare Triage for crates.io release.
...
- Added global metadata (`homepage`, `documentation`, `keywords`, `categories`) to `[workspace.package]` in `Cargo.toml`.
...
- HEAD — chore: implement pre-bundled crates.io packaging and CI/CD manual dispatch workflow
