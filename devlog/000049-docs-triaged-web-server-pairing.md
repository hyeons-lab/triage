...
- 2026-06-02T20:18-0700 `crates/triaged/README.md` — Generalized the section to "Prebuilt desktop clients" covering macOS/Windows/Linux artifacts (per the multi-platform build added in PR #58), with correct binary names (`Triage.app`, `triage_client.exe`, `triage_client`). Added a prominent **unsigned binaries** note and per-OS run/enable steps: macOS quarantine removal (`xattr -dr com.apple.quarantine` / right-click Open / Privacy & Security), Windows SmartScreen "Run anyway", and Linux `chmod +x` + Secret Service requirement.
