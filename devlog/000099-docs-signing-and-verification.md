...
- 2026-07-22T07:30-0700 Copilot found a second real defect in the same code block: the
  `.sha256` sidecar is generated by `publish.yml:637-638` as `( cd "$dir" && sha256sum
  "$base" > "$base.sha256" )`, so it records only the asset's **basename**, and
  `sha256sum -c` therefore only resolves the asset when run from the directory holding
  both. Verified: from the download dir `shasum -a 256 -c asset.bin.sha256` prints
  `asset.bin: OK`; from the parent, `shasum: asset.bin: No such file or directory` and
  `FAILED open or read`. The docs now say to put the asset and both sidecars in one
  directory and lead the block with `cd`. Both Copilot findings on this branch were about
  a *command that does not run as written* — the local reviewers checked whether the
  prose was true, not whether the recipe executes. Worth copying the commands into a
  scratch dir and running them when a doc's payload is a command.
...
- 3e40eb5 — docs: give the macOS checksum command its file argument
- HEAD — docs: note that checksum verification must run from the download directory
