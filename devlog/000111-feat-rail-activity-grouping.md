...
- 2026-07-30T19:35-0700 Round 8, `flutter/triage_client/lib/main.dart`: the rail
  is a widgets-level `ReorderableList` behind a `GlobalKey<ReorderableListState>`
  instead of material's `ReorderableListView`, and `_regroupRail` calls
  `cancelReorder()` before rebuilding. `_railDragInFlight`/`_railRegroupDeferred`
  and the `onReorderStart`/`onReorderEnd` plumbing are gone with it;
  `_railDragProxyDecorator` replaces the lift shadow the material list supplied.
  `_railGroupingIsCollapsed` replaces the bare degraded flag as the drag guard.
  `_loadDaemonSessionInto` carries `repoRoot` and `worktreeRoot` as a pair.
- 2026-07-30T19:35-0700 Round 8, `session_rail_layout.dart`: `resolveRailReorder`
  takes `newIndex` in post-removal coordinates and no longer converts;
  `session_rail_layout_test.dart` retranslated to match.
- 2026-07-30T19:36-0700 Round 8, `crates/triaged/src/session.rs`:
  `demoting_dead_live_session_carries_its_activity_into_the_manifest` demotes
...
- 2026-07-30T19:30-0700 **Round 8: the drag deferral was inverted, and both
  reviewers found it independently.** `SliverReorderableListState._dragEnd` calls
  `onReorderEnd` at pointer-up (reorderable_list.dart:975) and only reaches
  `onReorder`/`onReorderItem` from `_dropCompleted` after the drop animation
  (:1018). Round 7's `_railDragInFlight` flag was released on `onReorderEnd`, so
  the re-group it deferred still ran *before* the drop it was deferring for. The
  branch's own test passed on a timing artifact: with 200ms pumps the drop
  animation is still at value 0, so `reverse()` completes synchronously and the
  reorder fires inside `gesture.up()`. Pump the drop properly and the order
  inverts.
- 2026-07-30T19:30-0700 Worse, `cancelReorder()` (:869) and `_dragCancel` (:936)
  call neither callback, so a flag cleared only on drag end sticks set forever
  after a pointer cancel, freezing re-grouping for the rest of the session.
- 2026-07-30T19:32-0700 First replacement attempt was a drag-start snapshot of
  the rail, resolving the drop against the list the row was picked up from and
  letting re-groups run freely. That surfaced a **pre-existing latent crash**:
  `ReorderableListView` wraps each child in
  `_ReorderableListViewChildGlobalKey(item.key, state)`, keyed on the `ValueKey`
  alone, and reordering the rows mid-drag duplicates it, which throws
  "Duplicate GlobalKey detected in widget tree". Rebuilding the rail during a
  drag is not safe at all, so the snapshot was the wrong shape.
- 2026-07-30T19:35-0700 Settled on the framework's own answer: hold a
  `GlobalKey<ReorderableListState>` and call `cancelReorder()` before
  re-grouping, which its doc asks for in exactly these words ("should be called
  before any major changes to the item list occur"). That required swapping
  material's `ReorderableListView` for the widgets-level `ReorderableList`, whose
  state is public where `_ReorderableListViewState` is private, and which keys
  its children `(subKey, index, state)` so a reorder cannot duplicate them. The
  material extras dropped were already replaced by the rail:
  `buildDefaultDragHandles` was off, and the lift shadow is now supplied by
  `_railDragProxyDecorator`. Cost: a re-group during a drag loses the gesture.
  That is visible and repeatable, where a drop resolved against the wrong list is
  silent and wrong.
- 2026-07-30T19:36-0700 Migrated to `onReorderItem` (user's call when asked).
  `_handleReorderItem` applies the pre-removal adjustment itself and skips the
  callback when the drop resolves to `oldIndex`, so `resolveRailReorder` lost its
  hand-written conversion. Every index bug on this branch lived in that
  conversion or downstream of it. `session_rail_layout_test.dart` was retranslated
  into post-removal coordinates and mutation-checked by re-inserting the old
  conversion: 4 tests fail with it back.
- 2026-07-30T19:37-0700 `_railGroupingDegraded` is no longer the drag guard on
  its own. It stays set until a context *fetch* succeeds, but
  `session_context_updated` pushes also carry repository roots, so the rail could
  be showing real headers over real groups while every drag silently did nothing.
  Replaced by `_railGroupingIsCollapsed`, which additionally requires the rail to
  have collapsed to a single group, the state that is actually indistinguishable
  from a genuine single-repository rail.
- 2026-07-30T19:38-0700 Em dashes removed from every line this branch adds, per
  the standing rule: 106 source lines, 124 devlog lines, and all 13 commit
  messages (rewritten with `filter-branch --msg-filter`, safe because nothing is
...
- A re-group landing mid-drag now cancels the gesture rather than corrupting it.
  Preserving the drag instead would need a reliable "drag is fully over" signal,
  which `ReorderableList` does not offer (see the round 8 decisions). If losing
  the occasional drag proves annoying in practice, the fix is upstream: a
  callback that fires on cancel as well as on drop.
