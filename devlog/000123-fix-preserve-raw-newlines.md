...
Fix `antigravity` (`agy` CLI) logo banner and prompt rendering corruption caused by forced `\n` -> `\r\n` newline translation in the daemon and web client terminal pipeline. Filter synthetic terminal emulator query auto-responses to prevent phantom keystroke injection and session freezes.
...
- `flutter/triage_client/lib/widgets/terminal_pane_web.dart` — Removed `convertEol: true`.
...
- HEAD — fix: remove web convertEol and filter emulator query responses
