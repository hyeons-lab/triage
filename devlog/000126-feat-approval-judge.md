...
2026-08-19T11:18-0700 `crates/triage-core/src/config.rs`, `crates/triage-hook/src/main.rs`, `crates/triaged/src/judge.rs` — addressed fourth-round PR review comments: validated `JudgeConfig` rules unconditionally regardless of `enabled` flag, handled zero-length quoted environment values in `strip_leading_env_vars` (e.g. `FOO=""`), and added test coverage for subshell substitutions and unreachable IPC socket fallbacks.

2026-08-19T11:40-0700 `crates/triage-core/src/session.rs`, `crates/triaged/src/session.rs`, `crates/triaged/src/ipc.rs`, `crates/triage-transport-ws/src/lib.rs`, `crates/triage-hook/src/main.rs` — addressed fifth-round PR review comments: returned updated `SessionJudgePolicy` directly from `set_session_judge_policy` across `SessionApi`, IPC, and WebSocket transport to eliminate redundant policy queries; handled attached `-uVAR` and `-C/path` flags in `strip_leading_env_vars` in `triage-hook`; and validated against shell metacharacters (`;&|\n\r$\``) in `add_judge_allow_command`.

2026-08-19T11:57-0700 `scripts/install.sh`, `crates/triaged/src/service.rs` — implemented auto-approval out-of-the-box provisioning improvements from plan: signed staged binaries before atomic replace on macOS with verification, provisioned hooks to full list of Antigravity, Claude Code, and workspace paths using `$BIN_DIR/triage-hook`, called `install_global_agent_hooks()` at start of `triaged reload`, and updated install completion message to recommend `triaged reload`.

2026-08-19T12:03-0700 `crates/triaged/src/main.rs`, `crates/triaged/src/service.rs` — ensured global agent lifecycle hooks are automatically provisioned and verified on daemon startup in `triaged::main::run`, enabling out-of-the-box auto-approval without requiring any manual installation commands.

2026-08-19T12:06-0700 `crates/triage-hook/src/main.rs`, `crates/triaged/src/judge.rs` — addressed sixth-round PR review comments: supported inline equals flags (`--chdir=DIR`, `--unset=VAR`) and `-S` flag in `strip_leading_env_vars` in `triage-hook`, added unit test for in-process offline rule fallback under broken socket, and added subshell/heredoc pipe unit tests in `judge.rs`.

2026-08-19T13:00-0700 `crates/triaged/src/summarizer.rs`, `crates/triaged/src/session.rs`, `crates/triaged/src/judge.rs`, `crates/triage-hook/src/main.rs`, `crates/triaged/src/service.rs` — addressed max-effort review-fix-loop findings: logged preempted detail summary cancellations at `debug` instead of `warn`, moved synchronous disk persistence in `mutate_judge_config` off-lock with full `validate()` checking, converted direct stdout `println!` in background hook installation to structured `tracing::info!`, fixed git allow rule fallthrough to user-custom allow commands, tightened `for` loop syntax validation in compound commands, and aligned `BUILTIN_ALLOW_COMMANDS` with active deterministic rules.

2026-08-19T13:08-0700 `crates/triaged/src/judge.rs`, `crates/triaged/src/session.rs`, `crates/triage-core/src/config.rs` — separated custom allow commands from builtin tables so user-configured git rules (e.g. `git lfs pull`) match in Layer 2 without allowing built-in mutating flag bypasses, included `is_empty()` for bare `git branch` matching, normalized whitespace in custom deny substrings, added nanosecond timestamps to atomic persistence temp paths to eliminate intra-process collision, made `JudgeConfig::validate` public, and held lock during mutation in `mutate_judge_config`.

2026-08-19T13:13-0700 `crates/triaged/src/judge.rs`, `crates/triage-hook/src/main.rs` — cleaned up duplicate and obsolete doc comments on sensitive substrings and git global options, and simplified attached prefix check in `strip_leading_env_vars`.

2026-08-19T14:23-0700 `.agents/hooks.json` — removed checked-in repository hook to rely entirely on global `~/.agents/hooks.json` and prevent duplicate multi-hook execution in development workspaces.

2026-08-19T14:29-0700 `crates/triage-hook/src/main.rs` — improved escaped quote sequence handling in `strip_leading_env_vars`, locked stdin reader in `decide()`, and added unit tests for quote edge cases.

2026-08-19T14:43-0700 `crates/triage-core/src/judge_rules.rs`, `crates/triage-core/src/judge.rs`, `crates/triage-core/src/session.rs`, `crates/triage-hook/Cargo.toml`, `crates/triage-hook/src/main.rs`, `crates/triaged/src/judge.rs`, `crates/triaged/src/session.rs`, `crates/triage-transport-ws/src/lib.rs` — decoupled `triage-hook` from `triaged` to minimize binary size and eliminate heavy runtime dependencies; moved deterministic `JudgeRules` and evaluation tables into `triage-core::judge_rules`; implemented single-lock batch `list_session_judge_policies()` in `SessionApi` and `SessionManager` to eliminate N+1 mutex contention; lowered `JUDGE_TIMEOUT` to 3s with bounded socket read/write timeouts; added quote-aware backslash escape counting in `strip_leading_env_vars`; and added unit tests for severed socket fallback and compound operator safety.

2026-08-19T15:00-0700 `crates/triage-core/src/judge_rules.rs`, `crates/triage-hook/src/main.rs` — addressed PR review comments: hardened `matching_git_allow_rule` and `has_write_git_subcommand` to reject mutating flags and creation in `git branch` while allowing read-only inspection queries (`git branch --list`, `-r`, `-a`), normalized relative dot components in `check_target_credential_path` to block traversal evasions (`./src/../../.env`), and added unit test coverage for socket timeout stalls and credential path traversals.

2026-08-19T15:11-0700 `crates/triage-core/src/ipc.rs`, `crates/triage-core/src/judge_rules.rs`, `crates/triage-hook/src/main.rs`, `crates/triaged/src/ipc.rs`, `crates/triaged/src/session.rs`, `scripts/install.sh`, `scripts/antigravity_review.py` — consolidated canonical `default_socket_path()` and path sanitization in `triage-core::ipc` to eliminate socket path resolution divergence between daemon and hook; standardized self-healing `.unwrap_or_else(|p| p.into_inner())` mutex recovery across all `SessionManager` judge methods; supported standard duration suffixes in wrapper command argument parser for `timeout`; removed dirty-worktree `$ROOT/.agents/hooks.json` write destination from `scripts/install.sh`; and added standalone `--local` review execution to `scripts/antigravity_review.py`.

2026-08-19T15:16-0700 `crates/triage-core/src/judge_rules.rs`, `crates/triaged/src/session.rs`, `docs/approval-judge.md` — pre-allocated segment buffers in `pipeline_and_chain_segments`, updated `docs/approval-judge.md` to document decoupled hook architecture, keybindings (`a`/`S`), and Windows offline rule fallback, and added test coverage for session shutdown judge override cleanup.

2026-08-19T15:25-0700 `crates/triage-core/src/session.rs`, `crates/triaged/src/session.rs` — forwarded `list_session_judge_policies()` in `Arc<dyn SessionApi>` implementation to invoke `SessionManager`'s single-lock batch snapshot method instead of falling back to default $N+1$ trait loop.

2026-08-19T15:29-0700 `crates/triage-core/src/judge_rules.rs` — resolved canonical paths in `persist_judge_config` to preserve dotfile symlinks when atomically writing judge configuration, and added unit test coverage.

2026-08-19T15:37-0700 `Cargo.toml`, `crates/triage-core/Cargo.toml`, `crates/triage-core/src/ipc.rs`, `crates/triage-hook/src/main.rs`, `crates/triaged/src/judge.rs` — switched Unix UID resolution to authoritative `libc::getuid()`, enhanced `triage-hook` permission overrides generation to parse git subcommands across global flags (`git -C /path status`), and added unit test coverage for command substitutions and quoted metacharacters in commit messages.

2026-08-19T15:44-0700 `crates/triage-core/src/judge_rules.rs`, `crates/triage-hook/src/main.rs`, `crates/triaged/src/judge.rs`, `crates/triaged/src/session.rs` — restricted `tool(...)` permission overrides strictly to verified read-only tools (`is_read_only_tool`), added shell startup profiles (`.bashrc`, `.zshrc`, `.profile`) and system config files (`/etc/passwd`, `/etc/hosts`) to sensitive path protections, collapsed relative parent traversal components (`..`) lexically in credential path checks, and ensured on-disk judge configuration persistence succeeds before in-memory state mutation in `SessionManager`.

2026-08-19T15:55-0700 `crates/triage-core/src/judge_rules.rs`, `crates/triage-hook/src/main.rs`, `crates/triaged/src/service.rs`, `crates/triaged/src/session.rs`, `crates/triaged/src/summarizer.rs` — switched Claude Code hook serialization to expected snake_case field format, added pre-decode judge queue check in `run_summarize_job` to eliminate one-line summary preemption delay, ensured `JudgeState` is always initialized on daemon startup for seamless config mutations even when `enabled = false`, pruned duplicate global agent hook provisioning across plugin dirs, and added `.claude.json`, `.claude/settings.json`, and `.config/gcloud` to credential path protection.

2026-08-19T16:05-0700 `crates/triage-core/src/ipc.rs`, `crates/triage-core/src/judge_rules.rs`, `crates/triage-hook/src/main.rs` — aligned `triage-hook` IPC timeout to 10s to match daemon 8s inference bound, prefixed Windows named pipe path with `\\.\pipe\`, and added regression tests for global git flags (`git -C`, `git -c`) and complex leading variable assignment quoting.

2026-08-19T16:56-0700 `crates/triage-core/src/judge_rules.rs`, `crates/triage-hook/src/main.rs` — introduced quote-aware word tokenizer (`tokenize_words`) for command segments to correctly parse quoted env var assignments without splitting mid-value, trimmed and guarded empty pipeline segments, switched `triage-hook` permission overrides deduplication to $O(N)$ `HashSet`, and added regression tests for chained commands with empty segments and sensitive needles.

2026-08-19T17:14-0700 `crates/triage-core/src/judge_rules.rs`, `crates/triage-hook/src/main.rs`, `crates/triaged/src/service.rs`, `docs/approval-judge.md`, `scripts/antigravity_review.py` — switched `tokenize_words` to character-based iteration preserving multi-byte UTF-8 Unicode codepoints, implemented JSON hook merging in `install_global_agent_hooks` for pre-existing agent configurations, synced documentation keybindings for TUI settings overlay (`F5`), and aligned FlatBuffers generated path exclusions in CI review scripts.

2026-08-19T17:30-0700 `crates/triage-core/src/ipc.rs`, `crates/triage-core/src/judge_rules.rs`, `crates/triage-hook/src/main.rs` — expanded `.env*` credential file matching to cover hyphenated and underscored variants (`.env_production`, `.env-secret`), secured Unix temp socket directory permissions with mode `0o700`, and bounded hook standard input ingestion with timeout channel to prevent unclosed pipe hangs.

2026-08-19T18:05-0700 `crates/triage-core/src/judge_rules.rs`, `crates/triage-hook/src/main.rs`, `crates/triaged/src/judge.rs`, `docs/approval-judge.md` — fortified credential path traversal normalization for system files (`/etc/passwd`, `/etc/shadow`, `/etc/sudoers`, `/etc/hosts`), populated `builtin_deny_substrings()` with sensitive needles for client rule inspection, robustly handled value-taking global git options in `has_write_git_subcommand`, and updated shim timeout documentation to 10s.

2026-08-19T18:09-0700 `crates/triage-core/src/judge_rules.rs`, `crates/triage-hook/src/main.rs`, `crates/triaged/src/session.rs` — cleaned up temp file on rename errors in `persist_judge_config`, optimized `set_session_judge_policy` lock acquisitions by constructing policy directly, documented stdin reader thread teardown semantics, and added compound command regression tests with null redirections and unicode strings.

2026-08-19T18:12-0700 `crates/triage-hook/src/main.rs`, `crates/triaged/src/session.rs`, `scripts/install.sh` — harmonized uninitialized judge policy fallback to `true` in `list_session_judge_policies`, canonicalized config path resolution in hook offline evaluation, and aligned `install.sh` hook destinations with root agent configuration paths.

2026-08-19T18:17-0700 `crates/triage-core/src/judge_rules.rs`, `crates/triage-hook/src/main.rs` — expanded built-in credential path denylist to protect developer and CLI secrets (`.cargo/credentials*`, `.aws/config`, `.dockercfg`, `.vault-token`, `.config/op`), consolidated `normalize_lowered` string loop into a single pass, and dynamically propagated remaining timeout budgets into `ask_daemon` to strictly guarantee execution completes within agent hook limits.

2026-08-19T18:24-0700 `crates/triage-hook/src/main.rs`, `crates/triaged/src/handover.rs`, `crates/triaged/src/handover_tests.rs`, `crates/triaged/src/session.rs` — serialized Claude Code PreToolUse response output using `camelCase` schema (`hookSpecificOutput`, `permissionDecision`, `permissionDecisionReason`), dropped judge lock across synchronous disk I/O in `mutate_judge_config`, preserved per-session judge overrides across zero-downtime daemon handover, and added regression tests.

2026-08-19T18:29-0700 `crates/triage-core/src/config.rs`, `crates/triage-core/src/judge_rules.rs` — unconditionally validated `timeout_ms > 0` in `JudgeConfig::validate()`, updated `strip_null_redirections` to preserve null redirection patterns within quoted string literals, and pre-allocated token vectors in `tokenize_words`.

2026-08-19T18:33-0700 `crates/triage-core/src/judge_rules.rs` — created parent directory before canonicalizing path in `persist_judge_config`, and cached pre-tokenized `BUILTIN_ALLOW_COMMANDS` via `std::sync::OnceLock`.

2026-08-19T18:37-0700 `crates/triage-core/src/judge_rules.rs` — supported grouped parenthesis and brace segment splitting in `pipeline_and_chain_segments`, and stored static slice reference in `JudgeRules` to eliminate table allocations.

2026-08-19T18:41-0700 `crates/triage-core/src/judge_rules.rs`, `crates/triage-hook/src/main.rs`, `crates/triaged/src/session.rs` — added JSON value string fallback scanning in hook `extract_path`, serialized administrative on-disk config mutations with `judge_config_mutation_lock`, and optimized heap allocation in `check_target_credential_path`.

2026-08-19T18:46-0700 `crates/triage-core/src/judge_rules.rs` — supported attached git global flags in `parse_git_subcommand` and enforced alphabetic-only requirement in `is_short_flag_bundle_containing`.

2026-08-19T18:51-0700 `crates/triage-core/src/judge_rules.rs`, `crates/triage-hook/src/main.rs` — treated unclosed quotes and unfinished escapes as complex metacharacters in `has_complex_shell_metacharacters`, parsed literal backslashes inside single-quoted env values in `strip_leading_env_vars`, and cached built-in command/deny tables with `OnceLock`.

2026-08-19T18:55-0700 `crates/triaged/src/judge.rs`, `crates/triaged/src/session.rs` — inspected `.gemini/config/hooks.json` in `get_hook_status`, and prohibited redirection characters `<` and `>` in `add_judge_allow_command`.

2026-08-19T18:58-0700 `crates/triaged/src/service.rs` — implemented atomic write + rename in `install_global_agent_hooks` to prevent race conditions during concurrent daemon provisioning.

2026-08-19T19:02-0700 `crates/triage-core/src/config.rs`, `crates/triaged/src/session.rs` — validated shell control characters in `JudgeConfig::validate()`, and recovered poisoned mutex on `summarizer` in `decide_tool_call`.

2026-08-19T19:06-0700 `crates/triage-core/src/judge_rules.rs`, `crates/triage-hook/src/main.rs` — tokenized command segments using `tokenize_words` in hook permission overrides, stripped Windows drive letter prefixes in `check_target_credential_path`, and flushed physical storage with `sync_all()` prior to atomic rename in `persist_judge_config`.

2026-08-19T19:19-0700 `crates/triage-core/src/judge_rules.rs`, `crates/triage-hook/src/main.rs` — passed dynamic remaining timeout to `send_ipc_judge_request`, stripped Windows UNC/extended-length prefixes in `check_target_credential_path`, preserved backslashes in `unquote_segment`, and normalized escaped program tokens in `program_name`.

2026-08-19T19:42-0700 `crates/triage-core/src/judge_rules.rs`, `crates/triaged/src/judge.rs`, `crates/triaged/src/summarizer.rs` — used `atomic_write_file` in `configure_hook`, added preemption checks against pending judge jobs before summary inference in `summarizer.rs`, and bounded oversized character counting with `.take()` in `MAX_COMMAND_CHARS`.

2026-08-19T19:59-0700 `crates/triage-core/src/ipc.rs`, `crates/triage-core/src/judge_rules.rs`, `crates/triage-hook/src/main.rs`, `docs/approval-judge.md` — addressed PR review comments: enforced 0o700 permissions on XDG runtime directory creation, added process substitution checks (`<(`, `>`) to `has_complex_shell_metacharacters`, added `--config-env`/`--work-tree` to disqualifying git global arguments, capped stdin read buffer to 2MB with explicit `std::process::exit(0)` in `triage-hook`, and documented model reasoning block constraints in `docs/approval-judge.md`.

2026-08-19T20:33-0700 `crates/triage-core/src/ipc.rs`, `crates/triage-core/src/judge_rules.rs` — disallowed mutating `gh api graphql` endpoint in `matching_gh_allow_rule` with regression unit test, and verified directory creation validity on `XDG_RUNTIME_DIR`.

2026-08-19T20:44-0700 `crates/triage-core/src/ipc.rs`, `crates/triage-core/src/judge_rules.rs` — disqualified git execution-modifying global flags (`-p`, `--paginate`, `--config`) from allowlist with regression test, and added UID ownership and symlink validation on shared temp socket directory fallbacks.

2026-08-19T20:49-0700 `crates/triage-core/src/config.rs`, `crates/triage-core/src/ipc.rs`, `crates/triage-core/src/judge_rules.rs`, `crates/triage-hook/src/main.rs` — added parentheses `()` to forbidden shell characters in `JudgeConfig::validate()`, restricted `codesign` to verification flags in built-in allowlist, disqualified sensitive environment variables (`LD_PRELOAD`, `DYLD_INSERT_LIBRARIES`, `BASH_ENV`) in `export`, added fallback symlink/UID validation in IPC socket path resolution, and simplified hook permission overrides deduplication.

2026-08-19T21:12-0700 `crates/triaged/src/main.rs` — qualified zero-downtime reload description in CLI help for Unix.

2026-08-19T21:17-0700 `crates/triage-core/src/judge_rules.rs`, `crates/triage-hook/src/main.rs` — prevented stripping environment assignments containing command substitutions in `strip_leading_env_vars` with regression tests, and strengthened case-insensitive `graphql` and mutating flag checks in `matching_gh_allow_rule`.

2026-08-19T21:23-0700 `crates/triage-core/src/ipc.rs`, `crates/triage-core/src/judge_rules.rs` — fixed attached mutating flags evasion (`-d`, `-D`, `-f`, `-m`, `-a`, `-s`, `-u`) in `git tag` and `git branch` allowlist rules with unit tests, removed `manage_task` and `schedule` from `is_read_only_tool`, and used deterministic user state directory fallback in `default_socket_path`.
...
(and occasionally wrong) verdict across runs.
...
4b35508 — feat(triaged): judge routine agent tool calls with resident model and rule tables
a7019fb — feat(hook): add triage-hook shim with multi-format agent auto-detection and rule fallback
7d87a6e — feat(service): add triaged reload and automated multi-agent hook provisioning
aa59c03 — feat(triage): add per-session approval judge controls, status badges, and settings screen in TUI
HEAD — feat(triage_client): add tabbed settings dialog, approval traffic dashboard, and rule editor in Flutter
